...
{
	Index idxSrc = list->head;
	while (idxSrc != NullIndex) {
		Index idxDst = list->array[idxSrc].next;
		Index prev = idxSrc;
		while (idxDst != NullIndex) {
			if (compare(&list->array[idxSrc].data, &list->array[idxDst].data) == 0) {
				list->array[prev].next = list->array[idxDst].next;
				deleteNodeIndex(list, idxDst);
				idxDst = list->array[prev].next;
			}
			else {
				prev = idxDst;
				idxDst = list->array[idxDst].next;
			}
		}
		idxSrc = list->array[idxSrc].next;
	}
}
// N番目のノードインデックスを取得する
Index Retrieve(List* list, int no)
{
	...
	if (no >= 0) {
		idx = list->head;
		while (no > 0 && idx != NullIndex) {
			idx = list->array[idx].next;
			no--;
		}
	}
	return idx;
